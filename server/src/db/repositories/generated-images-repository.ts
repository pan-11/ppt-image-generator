...
export function createGeneratedImagesRepository(db: Database.Database) {
  return {
    ...
    listByBatchId(batchId: string) {
      return db.prepare("select * from generated_images where batch_id = ? order by created_at asc").all(batchId);
    },
    listByTaskId(taskId: string) {
      return db.prepare("select * from generated_images where task_id = ? order by created_at asc").all(taskId);
    },
    listByIds(imageIds: string[]) {
      ...
      return db.prepare(`select * from generated_images where id in (${placeholders}) order by created_at asc`).all(...imageIds);
    },
    getById(imageId: string) {
      return db.prepare("select * from generated_images where id = ?").get(imageId);
    },
