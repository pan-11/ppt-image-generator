...
describe("timeout retry", () => {
  ...
  it("continues polling an existing remote task after a transient fetch failure", async () => {
    const appDataDir = mkdtempSync(join(tmpdir(), "image-generator-fetch-failure-"));
    tempDirs.push(appDataDir);
    const service = new BatchService({
      envOverrides: {
        TOAPIS_API_KEY: "test-key",
