...
it("preserves source-bound child drafts and settings across collapse, switching and polling", async () => {
  ...
  await user.upload(screen.getAllByLabelText("上传补充参考图 1")[0], new File(["reference"], "ref.png", { type: "image/png" }));
  ...
  expect(screen.queryByRole("img", { name: "参考图 ref.png" })).not.toBeInTheDocument();
  ...
  expect(screen.getByRole("img", { name: "参考图 ref.png" })).toBeVisible();
