...
import { PageCandidateStrip, PageSelectionPanel } from "../components/courseware/page-selection-panel";
...
it("selects a grandchild instead of its original and copies the actual candidate prompt", () => {
  ...
  expect(screen.getByRole("radio", { name: "改图3 · 已选定稿" })).toBeChecked();
...
it("shows exact original and descendant source labels and retains them across candidate polling order changes", () => {
  ...
  expect(screen.getByText("改图3")).toBeVisible();
  expect(screen.getByText("基于原图1")).toHaveAttribute("title", "original.png · 子图提示词需配合参考图使用");
  expect(screen.getByText("基于改图2")).toHaveAttribute("title", "child.png · 子图提示词需配合参考图使用");
  rerender(<PageCandidateStrip candidates={[...candidates].reverse()} tasks={tasks.map((task) => ({ ...task }))} />);
  expect(screen.getByRole("button", { name: "查看 grandchild.png 大图" }).closest("article")).toHaveTextContent("基于改图2");
  expect(screen.getByRole("button", { name: "查看 child.png 大图" }).closest("article")).toHaveTextContent("基于原图1");
  expect(screen.getByText("改图3").closest("article")).toContainElement(screen.getByRole("button", { name: "查看 grandchild.png 大图" }));
  rerender(<PageCandidateStrip candidates={[{ id: "new", task_id: "original", filename: "new.png", local_path: "new" }, ...candidates]} tasks={tasks} />);
  expect(screen.getByText("原图4").closest("article")).toContainElement(screen.getByRole("button", { name: "查看 new.png 大图" }));
  expect(screen.getByText("改图3").closest("article")).toContainElement(screen.getByRole("button", { name: "查看 grandchild.png 大图" }));
});
