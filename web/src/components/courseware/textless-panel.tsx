...
import { createTextlessRun, downloadPptx, exportFilename, fetchTextlessRun, restoreTextlessRun, listTextlessRuns, type CoursewareDetail, type CoursewareDocument, type TextlessDetail, type TextlessRun } from "../../lib/courseware-api";
...
export function TextlessPanel(props: { open: boolean; document: CoursewareDocument; sourceDetail?: CoursewareDetail | null; models: ModelOption[]; flush: () => Promise<CoursewareDocument | null>; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const poll = async (restore = false) => {
      try { const result = await (restore ? restoreTextlessRun(selectedId) : fetchTextlessRun(selectedId)); if (!cancelled && !retrying.current) setDetail(result); }
      catch (cause) { if (!cancelled) setError(cause instanceof Error ? cause.message : "读取去字结果失败"); }
      if (!cancelled) timer = setTimeout(() => void poll(), 2000);
    };
    setDetail(null); void poll(true);
  ...
  const retryPages = async (taskIds: string[]) => {
    if (!detail || !taskIds.length || retrying.current) return;
    const operation = epoch.current;
    const runId = detail.run.id;
    retrying.current = true;
    setBusy(true); setError("");
    try {
      await retryTasks([...new Set(taskIds)]);
    ...
    finally { retrying.current = false; if (operation === epoch.current) setBusy(false); }
  };
  const failedPages = detail?.pages.filter(page => ["failed", "unknown"].includes(page.status)) ?? [];
  ...
  return <ModalDialog open={props.open} label="无文字版本" className="textless-modal" onClose={() => { if (!busy) props.onClose(); }}>
    ...
    <label className="stacked">处理记录<select disabled={busy} value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">选择处理记录</option>{runs.map((run, index) => <option key={run.id} value={run.id}>{index === 0 ? "最近一次" : `记录 ${index + 1}`} · {run.manifest.length} 页 · {run.model}</option>)}</select></label>
    {detail ? <>
      <p>下面的两份 PPT 使用这次处理时保存的相同页序和定稿版本。</p>
      <div className="toolbar"><button className="ghost-button" disabled={busy} onClick={() => void exportRun("final")}>导出本次定稿 PPT</button><button className="primary-button" disabled={busy} onClick={() => void exportRun("textless")}>导出无文字 PPT</button><button className="ghost-button" disabled={busy || !failedPages.length} onClick={() => void retryPages(failedPages.map(page => page.taskId))}>重试失败页（{failedPages.length}）</button></div>
      <p className="muted-copy">只重试本次记录中的失败页，成功页保留；需要重新生成的失败页会按中转站规则计费。</p>
      {detail.pages.map((page) => {
        ...
        return <article className="textless-page" key={page.pageId}>
          ...
          <div className="toolbar">{["failed", "unknown"].includes(page.status) ? <button className="ghost-button" disabled={busy} onClick={() => void retryPages([page.taskId])}>重试这一页</button> : null}<button className="ghost-button" disabled={busy || changed || !currentPage} onClick={() => void process(page.pageId)}>重新去字这一页</button></div>
        </article>;
      })}
    </> : null}
