...
export function PromptLibraryModal(props: { open: boolean; document: CoursewareDocument; onClose: () => void }) {
  ...
  return <ModalDialog open={props.open} label="提示词库" className="prompt-library-modal" onClose={props.onClose}>
    <div className="panel-heading dialog-heading"><h3>提示词库</h3><button className="ghost-button" data-modal-initial-focus onClick={props.onClose}>关闭</button></div>
    ...
    <p className="muted-copy prompt-library-helper">{mode === "original" ? "完整保留最初导入的内容，后续编辑不会覆盖原文。" : "包含所有页面，按当前页序导出；不受参与 PPT 导出的勾选影响。"}</p>
